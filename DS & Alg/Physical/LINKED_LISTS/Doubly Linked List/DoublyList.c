...
void reverse(Doubly_List *pl) 
{
    ...
    Doubly_Node *current = pl->head;
    Doubly_Node *temp = NULL;

    // Traverse the list and swap the next and pre pointers for each node
    while (current != NULL) 
    {
    ...
    if (temp != NULL) 
    {
        pl->head = temp->pre;  // Update the head to the new first element
    }
}
